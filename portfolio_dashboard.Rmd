...
numericInput("shares2", label = "# of shares:", 50), id = 'inputGroup'), id = 'heightGroup')
```
</br>
...
numericInput("shares3", label = "# of shares:", 100), id = 'inputGroup'), id = 'heightGroup')
```
</br>
...
dateInput("date5", label = "Purchase date:", '2016-04-04'), id = 'inputGroup'),
...
renderTable({
    ...
    return_func <- function(w, x, y, z){
        tmp <- tq_get(w, get = "stock.prices", from = z) %>%
            filter(date == min(date) | date == max(date)) %>%
            arrange(date) %>%
            summarise(ticker = w, shares = x,
                current_price = close[2],
                purchase_price = ifelse(y == 0, close[1], y),
                initial_value = x*purchase_price,
                current_value = x*close[2],
                initial_value_adj = x*(adjusted[1] - (close[1] - purchase_price)),
