...
renderPlot({
  ...
  group_by(date) %>%
  mutate(total_shares = sum(initial_value)/sum(close),
         delta_shares = ifelse(is.na(sum(total_shares) - lag(sum(total_shares))), sum(total_shares), 
                               total_shares - lag(sum(total_shares)))) %>%
  pull(delta_shares)
